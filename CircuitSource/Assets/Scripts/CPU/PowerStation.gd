...
@export var TimeToChargeFor:float
@export var NeedPower: bool = false
@export var IsBatteryConnectedScreen: AnimatedSprite2D
@export var SwitchAudio: AudioStreamPlayer2D
...
func SetPowerUsage():
	...
	elif not ParentBatteryPlug.ChargeDecrease: ParentBatteryPlug.ChargeDecrease = 0
...
func IsBatteryAnimate() -> void:
	if ParentBatteryPlug.StoredItem: IsBatteryConnectedScreen.animation = "Connected"
	else: IsBatteryConnectedScreen.animation = "Disconnected"
...
func _process(delta: float) -> void:
	...
	if NeedPower:
		PowerTimer += delta
		if PowerTimer >= ResponseTime and not AbleToUsePower(): SceneManagment.Die()
	...
	if Input.is_action_just_pressed("Interact") and PlayerAbleToInteract: 
		...
		SwitchAudio.pitch_scale = randf_range(0.7,1.3)
		SwitchAudio.play()
...
func WaitThenChangePowerNeed() -> void:
	await get_tree().create_timer(TimeToChargeFor).timeout
	NeedPower = false
